module SmarterCSV
  ...
  def SmarterCSV.process(input, options={}, &block)
    ...
    csv_options = options.select do |key,val|
      [:col_sep, :row_sep, :quote_char].include? key
    end
    # options.slice(:col_sep, :row_sep, :quote_char)
...
#        puts "HeaderA: #{file_headerA.join(' , ')}" if options[:verbose]
      ...
      unless options[:user_provided_headers]
        ...
        if ! key_mappingH.nil? && key_mappingH.class == Hash && key_mappingH.keys.size > 0
          headerA.map! do |x|
            key_mappingH.has_key?(x) ? (key_mappingH[x].nil? ? nil : key_mappingH[x].to_sym) : (options[:remove_unmapped_keys] ? nil : x)
          end
        end
      end
      ...
      while !f.eof?
        break if options[:limit] && line_count > options[:limit].to_i

        # we can't use f.readlines() here, because this would read the whole
        ...
        # cater for the quoted csv data containing the row separator 
        # carriage return character in which case the row data will be split 
        # across multiple lines (see the sample content in spec/fixtures/carriage_returns_rn.csv)
        ...
        # remove empty values using the same regexp as used
        # by the rails blank? method
        ...
        if use_chunks
          ...
          if chunk.size >= chunk_size || f.eof? # if chunk if full, or EOF reached
            ...
            # the last chunk may contain partial data,
            # which also needs to be returned (BUG / ISSUE-18)
          end
